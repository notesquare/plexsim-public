...
Option | Description
------- | -----------
**--ui** | 시뮬레이션 진행 상황을 [rich](https://github.com/Textualize/rich)를 통해 그림과 같이 보여줍니다.  옵션을 끄지 않을 시 디폴트로 선택됩니다.![view](images/rich.png)
**--cycle** | 시뮬레이션 timestep 수를 지정합니다. `.h5` 파일을 입력으로 실행하는 경우에만 유효합니다.
...
```bash
python run.py --proj upload         # upload 실행
```
  ...
  -
    ...
    initial_condition:
      ...
      tracking:
        ...
        sample: random
```
각 입자에 대한 설정을 목록(`-`)으로 나누어 지정합니다. 각 입자는 하나의 `grid`를 만들어냅니다. 목록의 요소를 추가하거나 제거하여 몇 종류의 입자를 포함할지 선택할 수 있습니다.
- **species**: _{'electron', 'ion'}_
  - _'ion'_ 의 경우 현재는 양성자의 전하량과 질량으로 설정되며, 추후 전하량과 질량을 선택할 수 있도록 업데이트 예정입니다.
- **backend**: _{'cpu', 'gpu'}_
- **n_splits**: _list, optional._
  - 전체 `grid`를 x, y, z축을 따라 몇 개의 `subgrid`로 나눌지 지정합니다. `subgrid`는 MPI 환경에서 병렬화를 위한 기준으로, `Particel Mover`, `Moment Gathering` 등에서 입자에 관한 연산의 단위가 됩니다. 지정하지 않을 경우, [1, 1, 1]로 설정됩니다.
...
- **initial_condition**
  ...
  - **avg_velocity**: _float_
    - 입자의 초기 평균 속력을 지정합니다. 각 입자의 속력은 정규분포를 따르도록 설정됩니다.
  - **tracking**
    - **n_particles**: _int_
...
- **particles_spare_memory**: _float_
  - 입자 위치와 속도를 담고 있는 배열의 여유공간의 크기를 지정합니다. 초기 입자 수에 대한 비율로 설정되며, 이를 너무 작게 설정하여 주변 `subgrid`로 이동하는 입자 수가 남은 여유 공간의 크기보다 많을 경우 시뮬레이션이 중단됩니다.
...
## HDF-5
`HDF-5` 파일은 현재까지의 시뮬레이션 정보를 저장하고 있습니다. 각각의 `grid`가 하나의 [`HDF-5`](#grid-저장-그룹-구조) 파일을 만들고, 이를 총괄하는 Simulation [`HDF-5`](#simulation-저장-그룹-구조) 파일이 하나 있습니다.
...
`.h5` 파일을 읽어 시뮬레이션을 이어서 실행 시 `/settings`의 값을 읽어 환경을 load합니다. `/cycles`의 마지막 cycle에 저장된 field 및 입자 정보를 load하며 값이 없을 경우 실행되지 않습니다. 마지막 cycle 이후부터 이어서 값을 저장합니다.

- **/stats**: 매 cycle마다 저장되는 값으로 입자 수, 입자의 운동 에너지 총량 및 전/자기장에 의한 필드 에너지 합을 포함합니다.
- **/field**: `save_field_frequency`에서 지정한 주기마다 각 격자점의 전기장, 자기장 값을 저장합니다.
- **/grids/{grid_index}**: 입자의 위치와 속도를 저장합니다.
  - `save_particles_frequency`마다 `X`, `U` 테이블에 각각 전체 입자의 위치 및 속도를 저장하며, `@tracking_particles`에 추적하고 있는 입자의 `subgrid` 배열 내 인덱스와 `grid` 내 global_id를 key:value로 저장합니다.
  - `X`와 `U` 배열은 각 `grid` 저장 파일 내의 table을 가리키고 있는 virtual dataset입니다. 데이터를 확인하기 위해서는 `grid` 저장 파일이 같은 위치에 있어야 합니다.
  - `save_tracked_frequency`마다 추적하고 있는 입자의 위치, 속도를 `/tracked`그룹 내에 생성합니다. `tracking_ids`는 추적하고 있는 입자의 `grid` 내 global_id이며 순서는 `X`, `U` 배열의 인덱스와 일치합니다.
...
## Install
시뮬레이션 결과로 생성된 `.h5`파일을 읽어 시각화합니다. [Jupyter Notebook](https://github.com/jupyter/notebook)을 통해 실행할 수 있습니다. 사용 방법은 다음과 같습니다.
...
```bash
conda create -n (가상환경 이름) python=3.8.10     # 가상환경 생성
...
3. 주피터 노트북(또는 랩) 실행
(참고-주피터 노트북 사용법: https://planharry.tistory.com/22)
```bash
jupyter lab                                    # Jupyter lab 실행
```

4. Viewer 실행
`.h5` 파일과 `view.py` 파일이 있는 디렉토리에 `.ipynb` 노트북 파일을 새로 만들고 엽니다.   
셀을 만들고 다음 코드를 입력 후 실행합니다.
```python
...
snap
```
오류가 발생하지는 않는데 흰 화면에 아무 그림이 나오지 않는다면 브라우저를 새로고침해 보세요. 정상적으로 실행된 경우 다음과 같은 figure가 생성됩니다.
